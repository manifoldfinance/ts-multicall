...
describe('MultiCallService', () => {
  ...
  describe('callByGasLimit()', () => {
    it('Should load balances for the address', async () => {
      ...
      const callData = tokens.map((address) => ({
        to: address,
        data: provider.contractEncodeABI(ERC20ABI, address, 'balanceOf', [
          user,
        ]),
        gas: 30000,
      }));
      ...
      const res = await multiCallService.callByGasLimit(callData, gasLimit);
  ...
  describe('callByChunks()', () => {
    it('Should load balances for the address', async () => {
      ...
      const res = await multiCallService.callByChunks(callData);
